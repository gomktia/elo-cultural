...
const faseConfig: Record<FaseEdital, { label: string; className: string }> = {
  ...
  inscricao: { label: 'Inscrições Abertas', className: 'bg-green-50 text-[var(--brand-success)] border-green-100' },
  inscricao_encerrada: { label: 'Inscrições Encerradas', className: 'bg-pink-50 text-[var(--brand-secondary)] border-pink-100' },
  divulgacao_inscritos: { label: 'Divulgação Inscritos', className: 'bg-orange-50 text-[var(--brand-warning)] border-orange-100' },
  recurso_divulgacao_inscritos: { label: 'Recurso Divulgação', className: 'bg-orange-100 text-orange-800 border-orange-200' },
